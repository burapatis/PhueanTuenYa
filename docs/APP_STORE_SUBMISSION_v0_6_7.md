# App Store Connect — Submit for Review (v0.6.7)

เอกสารนี้เตรียมข้อมูลสำหรับกรอกใน **App Store Connect** ก่อนกด **Submit for Review**  
อ้างอิงจากโค้ดจริงในโปรเจกต์ PhueanTuenYa v0.6.7 (Build 69)
...
| **เวอร์ชัน** | 0.6.7 |
| **Build ที่ส่ง** | 69 (หรือ build ล่าสุดที่สถานะ Ready to Submit) |
...
### What's New in This Version (0.6.7)
```
• ปรับหน้าตั้งค่าให้เหมาะกับผู้ใช้และผู้ทดสอบมากขึ้น
...
```
(เว้นว่างได้ หรือใส่ URL เดียวกับ Support URL)
```
...
1. Launch the app — first-run consent screen appears (4 agreement cards in Thai).
2. Tap accept, then acknowledge the simple usage guide.
3. Home dashboard shows 6 modules: Medication, Blood Pressure, Feelings/Symptoms, Health Calendar, SOS, Settings.
4. Medication: add a medication item, set a reminder time, log "taken today". Local notification will fire at the scheduled time (please allow notifications when prompted).
5. Blood Pressure: add a reading (systolic/diastolic/pulse + optional note).
6. Feelings: select today's feeling and symptoms via tap choices.
7. Appointments: add a health appointment with optional reminder notification.
8. SOS: add a contact with phone number. Tap Call — a confirmation dialog appears before opening the Phone app (tel://). No auto-dial.
9. Settings: export CSV or PDF via Share Sheet (user-initiated only). View consent/guide again. Delete all data cancels pending notifications.
...
วิธีทดสอบ: เปิดแอป → รับทราบข้อตกลง → รับทราบวิธีใช้ → ทดสอบเพิ่มยา/ความดัน/อาการ/นัดหมาย/SOS/ส่งออก CSV-PDF
...
**หมายเหตุ:** ถ้าต้องการคมชัดขึ้น ถ่ายใหม่จาก Simulator iPhone 17 Pro Max ด้วย ⌘+S
...
- แสดงข้อตกลงและวิธีใช้ตอนเปิดครั้งแรก
...
- [ ] เลือก build **0.6.7 (69)** ในหน้าเวอร์ชัน
- [ ] กรอก Description, Subtitle, Keywords, Support URL, Copyright
- [ ] อัปโหลด Screenshots 1290×2796 ครบอย่างน้อย 3 รูป (แนะนำ 7 รูป)
...
- [ ] เปิดครั้งแรกเห็นข้อตกลง + วิธีใช้
...
*เอกสารนี้จัดทำสำหรับ PhueanTuenYa iOS v0.6.7 — อัปเดตเมื่อมีการเปลี่ยนฟีเจอร์หรือนโยบายความเป็นส่วนตัว*
